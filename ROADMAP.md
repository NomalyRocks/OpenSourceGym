...
- [x] Anti-passback (çift yönlü): içeride görünen üye giriş turnikesinden geçemez (`ALREADY_INSIDE`, yalnızca **erişilebilir** bir çıkış cihazı varken); her iki modda da üye başına 60 sn yeniden geçiş bekleme süresi (`ENTRY_COOLDOWN`) uygulanır — aksi halde çıkış QR'ının fotoğrafıyla katı kural sıfırlanabiliyordu. Simetrik olarak, **giriş kaydı olmayan üye çıkış turnikesini tetikleyemez** (`NOT_INSIDE`): kaydı olmayan üye zaten dışarıdadır, dolayısıyla reddetmek kimseyi içeride kilitlemez. Konum/paylaşım/abonelik kontrolleri ise yalnızca girişte uygulanır. Kapı açılmazsa bekleme süresi geri alınır.
- [x] Doluluk kaydı iki ayrı ömre sahip: `autoExitHours` **sayılmayı** sonlandırır, kayıt ise `max(24 saat, autoExitHours)` boyunca **saklanır**. Sayım penceresi dolduğu için kaydı silmek, hâlâ binada olan üyenin çıkış turnikesini tetikleyememesine yol açardı. Saklama süresi sayım penceresinden asla kısa olamaz: `autoExitHours` 48'e kadar ayarlanabildiği için sabit 24 saat, hâlâ sayılan bir kaydı silip aynı tuzağı geri getiriyordu. Kayıtları saatlik bir süpürücü temizler; temizlik artık panelin açılmasına bağlı değil.
...
- [x] **Her ret kayda geçer** — hız sınırına takılanlar dahil (`RATE_LIMITED`): iz bırakmayan bir ret incelenemez, üst üste gelen retler ise logun asıl göstermesi gereken örüntüdür. Hız sınırı satırı cihaz ve mesafe taşımaz; sınır, QR çözülmeden önce kasıtlı olarak devreye girer.
